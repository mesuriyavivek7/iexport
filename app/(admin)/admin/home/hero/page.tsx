...
import { useHero, useUpdateHero } from "@/hooks"
...
export default function HomeHeroPage() {
  ...
  useEffect(() => {
    if (!data) return
    setHeading(normalizeHeading(data.heading))
  ...
  const setTag = (i: number, value: string) =>
    setTags((prev) => { const next = [...prev]; next[i] = value; return next })
      ...
      {isLoading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <label htmlFor="hero-heading" className="block text-sm font-medium text-foreground">Heading</label>
                <textarea
                  id="hero-heading"
                  value={heading}
                ...
                />
                <p className="text-xs text-muted-foreground">Use a new line for the second part of the heading.</p>
              </div>

              <div className="space-y-2">
                <label htmlFor="hero-subheading" className="block text-sm font-medium text-foreground">Subheading</label>
                <input
                  id="hero-subheading"
                  type="text"
                  value={subheading}
                ...
                <div className="flex flex-col gap-2">
                  {(tags.length ? tags : [""]).map((tag, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      />
                      <Button type="button" variant="outline" size="icon" onClick={() => removeTag(i)} aria-label="Remove tag" className="shrink-0">
          ...
          <div className="flex flex-wrap gap-3 border-t border-border pt-6">
            ...
            >
              {updateHero.isPending ? <><Loader2 className="size-4 animate-spin" />Saving…</> : "Save changes"}
            </Button>
          </div>
        </form>
      )}
