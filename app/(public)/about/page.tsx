...
import { getAboutPageForPublic } from '@/lib/home-data'

const defaultBannerHeading = 'About Procure Export'
const defaultBannerSubheading =
...
const defaultSectionImage = '/assets/aboutpage.png'
const defaultSectionHeading = 'About Procure Export'
const defaultSectionP1 =
...
export default async function AboutPage() {
  const data = await getAboutPageForPublic()

  const bannerHeading = data?.bannerHeading ?? defaultBannerHeading
  const bannerSubheading = data?.bannerSubheading ?? defaultBannerSubheading
  const sectionImage = data?.sectionImage ?? defaultSectionImage
  const sectionHeading = data?.sectionHeading ?? defaultSectionHeading
  const sectionP1 = data?.sectionContentParagraph1 ?? defaultSectionP1
  const sectionP2 = data?.sectionContentParagraph2 ?? defaultSectionP2
  const visionTitle = data?.visionTitle ?? defaultVision.title
  const visionContent = data?.visionContent ?? defaultVision.content
  const missionTitle = data?.missionTitle ?? defaultMission.title
  const missionContent = data?.missionContent ?? defaultMission.content
  const ambitionTitle = data?.ambitionTitle ?? defaultAmbition.title
  const ambitionContent = data?.ambitionContent ?? defaultAmbition.content
      ...
      <section className='mx-auto max-w-6xl px-6 py-12 md:px-8 md:py-20'>
        <div className='grid grid-cols-1 lg:grid-cols-2 items-center gap-8 lg:gap-20'>
          <div className='relative w-full aspect-[4/3] lg:aspect-auto lg:min-h-[320px] overflow-hidden rounded-lg'>
            <Image
              ...
              unoptimized={sectionImage.startsWith('http')}
            />
          </div>
          <div className='flex flex-col gap-3 sm:gap-2'>
            <h2 className='md:text-base text-lg font-semibold leading-tight text-center md:text-left'>
              {sectionHeading}
            </h2>
        ...
        <div className='grid mt-24 lg:mt-32 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-stretch gap-16 md:gap-6'>
          <div className='relative rounded-2xl flex bg-[#f1f5f9] flex-col justify-center items-center p-6 sm:p-8 gap-2 sm:col-span-2 lg:col-span-1'>
            ...
            <h2 className='text-primary text-lg font-semibold'>{visionTitle}</h2>
          ...
          <div className='relative rounded-2xl flex bg-[#f1f5f9] flex-col justify-center items-center p-6 sm:p-8 gap-2'>
            ...
            <h2 className='text-primary text-lg font-semibold'>{missionTitle}</h2>
            <p className='text-center text-[#334155] text-sm sm:text-[15px] leading-6'>
              {missionContent}
            </p>
          </div>
          <div className='relative rounded-2xl flex bg-[#f1f5f9] flex-col justify-center items-center p-6 sm:p-8 gap-2 sm:col-span-2 lg:col-span-1'>
            ...
            <h2 className='text-primary text-lg font-semibold'>{ambitionTitle}</h2>
            <p className='text-center text-[#334155] text-sm sm:text-[15px] leading-6'>
              {ambitionContent}
            </p>
          </div>
            ...
            <div className='rounded-2xl bg-[#f1f5f9] p-6 sm:p-8 flex flex-col gap-4'>
              ...
              <div className='flex flex-col gap-3'>
                ...
                <div className='flex flex-wrap gap-2'>
                  <span className='text-primary text-xs sm:text-sm p-2 bg-gray-50 rounded-2xl px-3 sm:px-4'>
                    Jaggery
                  </span>
                  <span className='text-primary text-xs sm:text-sm p-2 bg-gray-50 rounded-2xl px-3 sm:px-4'>
                    Peanuts
              ...
              <div className='mt-4 pt-4 border-t border-[#cbd5e1]'>
                <div className='flex items-start gap-2'>
                  ...
                  <div className='flex flex-col gap-1 min-w-0'>
                    <span className='text-[#334155] font-medium text-sm sm:text-[15px]'>
                      Address:
                    </span>
                    <p className='text-[#334155] text-sm sm:text-[15px] leading-6'>
                      Plot No. 28/1, APMC Market, Visnagar
                      <br />
