...
export default async function CategoryPage({ params }: Props) {
  ...
  const categoryFromList = categories.find((c) => c._id === id)
  const categoryName =
    products.length > 0
        ...
        <section className='relative z-40 mx-auto max-w-6xl px-4 sm:px-6'>
          <div className='flex flex-col items-center gap-3 sm:gap-4'>
            ...
            <p className='text-center text-sm sm:text-base text-white leading-6 sm:leading-7 max-w-3xl'>
              Explore our premium {categoryName.toLowerCase()} products with
              consistent quality, reliable packaging, and export-ready standards.
            </p>
          </div>
        </section>
        <div
